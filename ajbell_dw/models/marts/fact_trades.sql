-- Fact table: trades. Grain = one row per trade (buy/sell event).
-- Holds measures (quantity, price, gross_amount) plus FOREIGN KEYS to dimension
-- surrogate keys. All descriptive detail lives in the dimensions, not here -
-- that thin-spine-of-keys shape is what makes it a star schema fact.
